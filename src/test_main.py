...
from src.instrument import code_instrumentation
from src.domain import mcmc_dom, multi_swarm
from src.runner import run
from src.GA.GA import GA
from src.GA.GA_Local import GA_Local
...
class domain_extraction:
    def __init__(self, input_path, input_file, func, output_path):
        ...
        self.extract_dot()
        # self.extract_pdf()
        self.extract_prime_path()
        self.extract_prime_path_coverage_bruteforce()
        self.extract_prime_path_coverage_setcover()
        self.extract_simple_path()
    ...
    def GA(self, p_path, boundary_delta, algorithm_parameters):
        print(p_path)
        ind_path = self.prime_coverage.index(p_path)
        requirement = self.prime_req[ind_path]
        aa = run(GA, "GA", self.input_path, self.file, self.function, self.output_path,
                 self.source, self.prime_pathes, requirement, p_path, boundary_delta, '', algorithm_parameters)
        dom_point = aa.execute()
        return dom_point

    def GA_Local(self, p_path, boundary_delta, algorithm_parameters):
        print(p_path)
        ind_path = self.prime_coverage.index(p_path)
        requirement = self.prime_req[ind_path]
        aa = run(GA_Local, "GA_Local",self.input_path, self.file, self.function, self.output_path,
                 self.source, self.prime_pathes, requirement, p_path, boundary_delta, '', algorithm_parameters)
        dom_point = aa.execute()
        return dom_point

    def multi_swarm_domain(self, p_path, boundary_delta, algorithm_parameters):
        print(p_path)
        ind_path = self.prime_coverage.index(p_path)
        requirement = self.prime_req[ind_path]
        aa = run(multi_swarm.MultiSwarmDom, "multi_swarm_domain", self.input_path, self.file, self.function,
                 self.output_path, self.source, self.prime_pathes, requirement, p_path, boundary_delta, '',
    ...
    def speciation_domain(self, p_path, boundary_delta, algorithm_parameters):
        print(p_path)
        ind_path = self.prime_coverage.index(p_path)
        requirement = self.prime_req[ind_path]
        aa = run(multi_swarm.SpeciationDom, "speciation_domain", self.input_path, self.file, self.function,
                 self.output_path, self.source, self.prime_pathes, requirement, p_path, boundary_delta, '',
                 algorithm_parameters)
        dom_point = aa.execute()
        # points = ["points"]
