...
class boundary_points:
    def __init__(self,  test_data, test_data_fitness, vartype, varbound, function, algorithm_parameters={"length": [10, 20], "budget": 20}):
        self.test_data = test_data
        self.test_data_fitness = test_data_fitness
    ...
    def boundary_for_oriantation(self, oriantation, delta):
        ori = oriantation
        ...
        while t < self.budget:
            ...
            if self.check_for_range(test_data) is True:
                score = self.fitness_function(test_data)[0]
                if (score != 0):
                    step_size = int(step_size / 2)
                    ori = reverse
                    flag = True
                    self.retracted_points.append([score, test_data])
                else:
                    self.boundary_test_data.append([score, test_data])
                    if flag is True:
                        step_size = int(step_size / 2)
                        ori = reverse
                        flag = False
                        self.non_retracted_points.append([score, test_data])
            t += 1

    def run_boundary_point(self, delta):
        for k in self.limit:
            self.length = k
            for i in self.oriantation:
                self.boundary_for_oriantation(i, delta)
            # extract to csv
            res = list()
        for i in self.boundary_test_data:
            if i not in res:
                res.append(i)
                if i[0] <= self.test_data_fitness:
                    self.test_data = i[1]
                    self.test_data_fitness = i[0]
        return res
