...
def pc_num_extract(source):
    # path condition and number of each condition
    _, line_cond_num = conditions(source)
    return line_cond_num


def line_true_false_v1(prime_pathes, output_dir):
...
def line_true_false(execution_path, output_dir):
    # in a prime path which line execute wgeb condition is true or false
    target_path = []
    with open(output_dir + "/CFG/Digraph.gv", "r") as f:
        lines = f.readlines()
    ...
    true_false_line = list()
    count = 0
    while count != len(execution_path) - 1:
        if [str(execution_path[count]), str(execution_path[count + 1]), "T"] in triple:
            true_false_line.append((execution_path[count], 'T'))
        elif [str(execution_path[count]), str(execution_path[count + 1]), "F"] in triple:
            true_false_line.append((execution_path[count], 'F'))
        count += 1
    return true_false_line
...
def extrac__dom_to_csv(list_data, name, output_path):
    # extract domain point of given path to the csv file
...
def dom_type(argums):
    ...
    for var, typ in argums.items():
        ...
        dom.append([float(low), float(high)])
        types.append([str(typ)])
        vars.append(str(var))
    return dom, types, vars
